...
bumped target in `release-targets.toml`, and `release.yml` publishes it. A `resolve`
job reads the tag's registry kind from the manifest, then one per-registry job runs
`cz publish "$TAG"`. Established packages publish via OIDC **trusted publishing** (no
