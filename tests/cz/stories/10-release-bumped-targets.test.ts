...
const stubFetch = ({ ghcrPublished, npmPublished, pypiEverVisible, pypiPublished }: {
  ...
  vi.stubGlobal('fetch', async (input: string | URL) => {
    ...
    if (url.includes('/token?')) return Response.json({ token: 'gh-token' });
    if (url.startsWith('https://registry.npmjs.org/')) return npmPublished ? ok() : notFound();
    if (url.startsWith('https://ghcr.io/v2/')) return ghcrPublished ? ok() : notFound();
    if (url.startsWith('https://pypi.org/')) {
      pypiCalls += 1;
      if (pypiCalls === 1) return pypiPublished ? ok() : notFound();
      return pypiEverVisible === false ? notFound() : ok();
    }
    return notFound();
  });
};
...
beforeEach(() => {
  ...
  git.revParse.mockImplementation(async (rev, flags) => {
    if (flags?.abbrevRef) return text(branchName);
    return text(rev === 'HEAD' ? headSha : remoteMainSha);
  });
  git.status.mockImplementation(async () => text(workingTreeStatus));
});
...
describe('10.3 publishing a pending target', () => {
  beforeEach(() => {
    stubFetch({ ghcrPublished: true, npmPublished: true, pypiPublished: false });
    gh.release.list.mockResolvedValue(text('false'));
    gh.run.list.mockImplementation(async ({ jq } = {}) => text(jq === '.[].databaseId' ? '100\n101' : '100\n101\n999'));
  });
  ...
  it('10.3.6 treats an empty known-run list as no prior runs', async () => {
    gh.run.list.mockImplementation(async ({ jq } = {}) => text(jq === '.[].databaseId' ? '' : '999'));
    gh.run.view.mockResolvedValueOnce(text('completed\nsuccess'));
    const cerberus = findTarget(await loadReleaseTargets(), 'zyplux-cerberus');
    const cerberusVersion = await cerberus.readVersion();
