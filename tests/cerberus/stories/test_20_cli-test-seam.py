...
from cerberus.checks import cli_test_seam_check
from cerberus.model import CheckResult, Finding, Repo, Status

RunCliTestSeam = Callable[[dict[str, str]], CheckResult]
...
_SEAM_OK = "every cli app exports only the root seam; story tests import only fixture aliases and node builtins"
...
@pytest.fixture
def run_cli_test_seam(monkeypatch: pytest.MonkeyPatch, repo: Repo, ctx: context.Context) -> RunCliTestSeam:
    def run(files: dict[str, str]) -> CheckResult:
        ...
        return cli_test_seam_check.run(repo, ctx)

    return run


def test_20_1_1_skips_repos_with_no_typescript_packages(run_cli_test_seam: RunCliTestSeam) -> None:
    result = run_cli_test_seam({})
    assert result.findings == [Finding(Status.SKIP, "no TypeScript packages")]


def test_20_1_2_skips_workspaces_with_no_cli_app(run_cli_test_seam: RunCliTestSeam) -> None:
    result = run_cli_test_seam({
...
def test_20_2_1_passes_a_cli_app_that_exports_only_the_root_seam(run_cli_test_seam: RunCliTestSeam) -> None:
    result = run_cli_test_seam({
        "package.json": _SEAM_ROOT_WS,
...
def test_20_2_2_fails_a_cli_app_that_declares_no_exports(run_cli_test_seam: RunCliTestSeam) -> None:
    result = run_cli_test_seam({
        "package.json": _SEAM_ROOT_WS,
...
def test_20_2_3_fails_and_names_each_export_beyond_the_root_seam(run_cli_test_seam: RunCliTestSeam) -> None:
    result = run_cli_test_seam({
        "package.json": _SEAM_ROOT_WS,
...
def test_20_2_4_fails_a_cli_app_whose_exports_omit_the_root_entry(run_cli_test_seam: RunCliTestSeam) -> None:
    result = run_cli_test_seam({
        "package.json": _SEAM_ROOT_WS,
...
def test_20_2_5_accepts_a_conditions_object_as_the_root_seam(run_cli_test_seam: RunCliTestSeam) -> None:
    result = run_cli_test_seam({
        "package.json": _SEAM_ROOT_WS,
...
def test_20_3_1_passes_story_tests_importing_only_fixture_aliases_and_node_builtins(
    run_cli_test_seam: RunCliTestSeam,
) -> None:
    result = run_cli_test_seam({
        "package.json": _SEAM_ROOT_WS,
        "apps/cli/package.json": _SEAM_CLI,
...
def test_20_3_2_fails_a_story_test_importing_the_app_package_directly(run_cli_test_seam: RunCliTestSeam) -> None:
    result = run_cli_test_seam({
        "package.json": _SEAM_ROOT_WS,
        "apps/cli/package.json": _SEAM_CLI,
        "tests/cli/package.json": _SEAM_TESTS_PKG,
...
def test_20_4_1_fails_an_imports_alias_that_escapes_the_test_package(run_cli_test_seam: RunCliTestSeam) -> None:
    result = run_cli_test_seam({
        "package.json": _SEAM_ROOT_WS,
        "apps/cli/package.json": _SEAM_CLI,
