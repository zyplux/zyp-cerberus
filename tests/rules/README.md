...
Black-box tests that exercise the **published** `@totvibe/eslint-config` surface — `totvibe()` and `plugin` — exactly as a downstream project would.

## Why the split

The home of a test follows what it exercises:

- **Here — config behaviour:** does the preset wire a rule with the right options? The rule setting is read off the public `totvibe()` array and run through a one-rule `Linter`. No package internals, so the test lives outside the package.
