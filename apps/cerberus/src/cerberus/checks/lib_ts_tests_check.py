...
Libraries are the published TypeScript workspace members without a `bin` —
cli apps carry their own seam under `cli-ts-tests`, private packages are not
importable surfaces, and packages without TypeScript sources (config-only
packages shipping JSON) have no internals to seal.
"""
...
def _is_library(manifest: dict[str, Any]) -> bool:
    return not manifest.get("bin") and not manifest.get("private")
...
def run(repo: Repo, ctx: Context) -> CheckResult:
    ...
    libraries = [m for m in members if _has_ts_sources(m, paths) and _is_library(seam.load_manifest(m))]
    if not libraries:
